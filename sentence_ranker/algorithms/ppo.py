...
def train_ppo(
    ...
    for step_idx in tqdm(range(shuffled_step_idxs), position=1):
        # Move batch to device if applicable
        input_ids = buffer.input_ids[step_idx].to(
            device
        )  # Shape: (num_candidates, max_seq_len)
        attn_masks = buffer.attn_masks[step_idx].to(
            device
        )  # Shape: (num_candidates, max_seq_len)
        logits = buffer.logits[step_idx].to(
            device
        )  # Shape: (num_candidates, max_seq_len, vocab_size)
        sft_logits = buffer.sft_logits[step_idx].to(
            device
        )  # Shape: (num_candidates, max_seq_len, vocab_size)
        candidate_masks = buffer.candidate_masks[step_idx].to(
            device
        )  # Shape: (num_candidates, max_seq_len)
        candidate_rewards = buffer.candidate_rewards[step_idx].to(
            device
        ...
        with torch.no_grad():
            ...
            step_returns: torch.Tensor = v_net(
                input_ids[-1],
                attn_masks[-1],
            ).logits.squeeze()  # Shape: (num_candidates)
            ...
            cache = DynamicCache()
            state_values_all = torch.zeros([num_candidates, max_seq_len], dtype=torch.float, device=device)
            for i in range(max_seq_len):
                ...
                state_values_all[:, i] = v_output.logits.squeeze()
            ...
            for i in reversed(range(max_seq_len)):
                ...
                prev_state_values = state_values_all[:, i] # Shape: (num_candidates)
        ...
        with torch.no_grad():
            ...
            old_act_log_probs = Categorical(logits=logits).logits # Shape: (num_candidates, max_seq_len, vocab_size)
            old_act_log_probs = old_act_log_probs[candidate_masks] # Shape: (num_tokens, vocab_size)
            old_act_log_probs = old_act_log_probs[torch.nn.functional.one_hot(actions, num_classes=vocab_size)] # Shape: (num_tokens)

        new_logits = p_net(input_ids, attn_masks).logits # Shape: (num_candidates, max_seq_len, vocab_size)
        new_logits[~candidate_masks] = 1.0
        new_act_distr = Categorical(logits=new_logits)
        new_act_log_probs = new_act_distr.logits  # Shape: (num_candidates, max_seq_len, vocab_size)
        new_act_log_probs = new_act_log_probs[torch.nn.functional.one_hot(actions, num_classes=vocab_size)] # Shape: (num_tokens)
