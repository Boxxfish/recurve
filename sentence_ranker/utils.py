...
def get_llm_scores(q_net: LlamaForCausalLM, input_ids: torch.Tensor, attn_masks: torch.Tensor) -> torch.Tensor:
    ...
    max_len = input_lens.amax().item() + 1
    q_vals = q_net.forward(input_ids.flatten(0, 1)[:, :max_len], attn_masks.flatten(0, 1)[:, :max_len]).logits.reshape(batch_size, num_candidates)
    return q_vals
